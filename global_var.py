...
def init_global(applogger, appmail, glbconfig):
    ...
    try:
        ...
        dbpool = PooledDB(MySQLdb, 5, **dbconf)
        logged = {}     # 已登录用户和token
        # 獲取selector
        enum_selector = get_selectors(dbpool)
    ...
    for tbi, tbv in selector_tbn_fn.items():
        ...
        for line in res:
            resenum[tbi][int(line[0])]={
                'id':int(line[0]),
                'val': line[1],
                'val_chs': base64.b64decode(line[2]).decode('utf8')
            }
    cur.close()
    conn.close()
    return resenum
...
def get_groups(dbpool):
    ...
    # groups
    resenum = []
    sqlstr = "select `id`,`smon`,`emon` from `group`;"
    conn = dbpool.connection()
    cur = conn.cursor()
    cur.execute("SET NAMES UTF8mb4;")
    cur.execute(sqlstr.encode("utf-8", "ignore"))
